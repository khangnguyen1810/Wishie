...
<task title="Target Branch Acquisition">
    EXECUTE below command with terminal tool to present questions to the user and collect [USER_TARGET_BRANCH] if the user hasn't yet provided:
    `sdlc-workflows ask-questions --questions "Please provide the target branch name to create PR:" --answers "E.g., 'main' or 'develop'" --expect-answer "Your Answer: "`
    ENSURE [USER_TARGET_BRANCH] is NOT EMPTY. IF EMPTY, STOP execution and REPORT error "Target branch name is REQUIRED to proceed."
</task>
</preWorkflowTasks>
