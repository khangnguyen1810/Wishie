...
- [x] **Scenario: Save Button Has Visible Bottom Padding and Is Not Obscured by Home Indicator** ✅ RESOLVED
  ...
  - **Resolution**: Wrapped the content `VStack` in `InterestsSelectionView` with a `GeometryReader` to read the safe area insets at runtime. Applied `.padding(.bottom, max(proxy.safeAreaInsets.bottom, 16))` to the `WishieButton` — this uses the device's actual safe area inset (e.g. ~34pt on iPhone 14+) when present, and falls back to 16pt on devices without a home indicator.
