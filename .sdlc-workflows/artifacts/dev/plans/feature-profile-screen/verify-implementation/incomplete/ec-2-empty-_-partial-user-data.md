...
- [ ] **Scenario: Profile row shows fallback dash when a string field is empty** ❌ FAILED
...
- [ ] **Scenario: Full Name row shows fallback when both firstName and lastName are empty** ❌ FAILED
...
- [ ] **Scenario: Date of Birth defaults to current date for user with no DOB stored** ❌ FAILED
