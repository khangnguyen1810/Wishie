...
- [x] **Scenario: Very long email address does not overflow or truncate label** ❌ FAILED
  ...
  - ✅ RESOLVED: `profileInfoRow` now uses `HStack` with label on the left (`minWidth: 110`) and value on the right with `.lineLimit(1)` and `.truncationMode(.tail)`. The `RoundedRectangle` background has been removed. Long email values truncate with an ellipsis without pushing the label off screen.

- [x] **Scenario: Full name with a very long first or last name wraps or truncates correctly** ❌ FAILED
  ...
  - **Affected Files**: [ProfileView.swift](../../../../../Wishie/Screens/Profile/ProfileView.swift) — `profileInfoRow` function (lines 75–91)
  - ✅ RESOLVED: `profileInfoRow` now uses `HStack` with label on the left (`minWidth: 110`) and value on the right with `.lineLimit(1)` and `.truncationMode(.tail)`. The `RoundedRectangle` background has been removed. A compound long name will truncate with an ellipsis at the trailing edge rather than wrapping or overflowing a fixed-height box.
