...
- **Known consequence of the above, called out explicitly**: for a wishlist that only exists in Postgres (created via the API, e.g. through Swagger), Home's and Archived's `archiveWishlist`/`unarchiveWishlist`/`deleteWishlist`/`leaveWishlist` actions call Firestore methods that target a document that doesn't exist — they're silent no-ops (the API-sourced wishlist list is simply refetched unchanged afterward). Tapping into such a wishlist also opens the Firestore-backed `WishlistDetailScreen`, whose `observeWishlist(by:)` listener never fires for a Postgres-only id, so it never loads/updates. This is expected and accepted for this transitional slice — full mutation support lands with `WishlistService`'s remaining Firestore→API migration in a later slice — but anyone testing against real API-created wishlists (rather than the empty-list default) should expect these actions to do nothing rather than error.
